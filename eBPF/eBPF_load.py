...
def load_programs(param2):
    ...
    #compilar
    command = "clang-11 -D COLLECT_SIZE="+str(param2)+" -target bpf -O2 -c eBPF/xdp.c -o eBPF/xdp.o"
    subprocess.call(command, shell=True)

    command = "clang-11 -D COLLECT_SIZE="+str(param2)+" -target bpf -O2 -c eBPF/tc.c -o eBPF/tc.o"
    subprocess.call(command, shell=True)
    ...
    #anexar programa XDP
    command = "sudo ip -force link set dev eth0 xdp obj eBPF/xdp.o sec xdp"
    subprocess.call(command, shell=True)
    ...
    command = "sudo tc filter add dev eth0 egress bpf da obj eBPF/tc.o sec tc"
    subprocess.call(command, shell=True)
