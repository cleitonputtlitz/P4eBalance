...
/*
  ...
  Pacote entrada: Eth, ipv4, tcp/udp, rpc
...
*/
  ...
  //bpf_custom_printk("carregou ip %d\n",iph->protocol);
  int tcp_len = 0;
  if(iph->protocol == TYPE_TCP) {
    //offset += sizeof(struct iphdr) + sizeof(struct tcphdr);
    //bpf_custom_printk("tipo tcp %d\n",sizeof(struct tcphdr));
    offset += sizeof(struct iphdr);
    struct tcphdr *tcp;
    tcp = (struct tcphdr *) (iph + 1); //(data + offset);
    if ((tcp + 1) > data_end)
      return XDP_PASS;
    //offset += sizeof(struct iphdr) + sizeof(struct tcphdr);
    ...
    offset += tcp->doff * 4 ;
    //bpf_custom_printk("\n ack %d", tcp->ack_seq);
    tcp_len = sizeof(struct ethhdr) + sizeof(struct iphdr) + (tcp->doff * 4) + 8;  // por exemplo, 64 bytes de payload
  }

  if(iph->protocol == TYPE_UDP) {
    offset += sizeof(struct iphdr) + sizeof(struct udphdr);
    //bpf_custom_printk("tipo udp");
  }


  if(tcp_len > 0) {
     // Garantir que este número de bytes esteja disponível
     if (bpf_skb_pull_data(skb, tcp_len) != 0) {
         //bpf_custom_printk("\n NAO CONSEGUIU CARREGOU MAIS DADOS\n");
         return TC_ACT_OK;  // Falha ao garantir que esses dados estejam disponíveis
     }
  ...
  if(!int_header_map) {
    bpf_custom_printk("FALHA ao buscar dados do map int_header rpc->id %d \n", rpc_id);
    return TC_ACT_OK;
  }
